...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            // )
            .csrf(csrf -> csrf
                .ignoringRequestMatchers("/api/login", "/api/logout", "/api/client_ticket", "/api/client_obtain_ticket", "/api/client_view_tickets", "/api/agent_home", "/api/agent", "/api/agent/tickets", "/api/agent/ticket/status", "/api/admin", "/api/admin_home", "/api/admin_dashboard", "/api/admin_users", "/api/admin_new_user", "/api/createUser", "/api/deleteUser") // Add /api/client_ticket
                .csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse())
            )
            .cors(cors -> cors.configurationSource(corsConfigurationSource()))
            .sessionManagement(session -> session
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/api/admin", "/api/admin_home", "/api/admin_dashboard", "/api/admin_users", "/api/admin_new_user", "/api/createUser", "/api/deleteUser").hasAuthority("ADMIN")
                .anyRequest().authenticated()
            )
            .formLogin(form -> form.disable())
            .logout(logout -> logout
